...
from visionary.util import ModuleState
...
class VKAPIHandle(object):
    ...
    async def register(self):
        ...
        self._listen_to = await self._get_chat_id(self._listen_to)
        ...
        if self._reply_to is None:
            ...
            self._reply_to = await self._get_chat_id(self._reply_to)
            self._log.debug(f"Replying to peer {self._reply_to}")

        await self._get_photo_upload_uri()
        self._state = ModuleState.ready
        self._log.info('VK API handle OK.')

    async def _get_chat_id(self, chat_name: str) -> Optional[int]:
        """
        Resolves chat names to its respective peer_ids.
        Returns:
